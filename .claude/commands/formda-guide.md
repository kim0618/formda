...
**발행 요일: 화요일 · 금요일 (주 2편)** - 2026-08-21 사용자 확정, 유틸 짝 가이드 4편 큐 기준.
...
- `u`가 **2·5**(화·금)이면 → 그대로 진행한다.
- `u`가 **1·3·4·6·7**(월·수·목·토·일)이면 → **글을 쓰지 말고 즉시 멈춰서 사용자에게 알린다.** 주제 선정도 `guides.js` 조회도 하지 않는다.

**비발행일 알림 문구 (그대로 사용):**
> 오늘은 YYYY-MM-DD **N요일**로 폼다 가이드 발행 지정 요일(화·금)이 아닙니다. 다음 발행일은 YYYY-MM-DD(요일)이고 예정 회차는 **N회차 {도구}**입니다. 그래도 지금 작성할까요? "진행"이라고 하시면 쓰겠습니다.

사용자가 "진행", "그래도 써", "오늘 써" 등으로 **명시적으로 지시하면 작성한다.** 사용자가 요일을 이미 알고 지시한 경우(예: "월요일인데 오늘 하나 써줘")도 명시 지시로 본다. 단, 알림 없이 임의로 진행하지는 않는다.

### 게이트 통과 후에도 확인할 것 2가지

1. **⛔ 온사이트 가이드 신규 생산은 2026-09-07부터 중단이다.** 발행일이든 큐가 남았든 쓰지 않는다. 아래 "재개 조건"이 충족되기 전에는 사용자에게 알리고 멈춘다.
   > 온사이트 가이드는 2026-09-07 실측으로 생산 중단 상태입니다. 재개 조건(구글 크롤 재개)이 아직 충족되지 않았습니다. 글 쓸 여력은 네이버 블로그(`~/formda/blog/queue.md`)로 보내는 것이 실측상 4배 이득입니다. 그래도 쓰시겠다면 "진행"이라고 해주세요.
2. **밀린 회차를 몰아 쓰지 않는다.** 지정일을 걸렀더라도 **한 발행일에는 1편만** 쓰고 나머지 회차는 다음 발행일로 민다. 만회하려고 2편을 몰아 쓰는 것이 정확히 피하려는 대량생산 패턴이다([[feedback_formda_guide_new_tool_only]]). 밀린 사실은 보고에만 남긴다.

### 🛑 생산 중단 판정 (2026-09-07 실측, 이 항목이 위 게이트의 근거)

10월 초로 미뤄뒀던 효과 판정을 **9/7 리포트에서 앞당겨 내렸다. 결과는 부정이다.** 아래는 GA4 28일(2026-08-10~09-06) 실측이다.

| 지표 | 값 |
|---|---|
| 외부 뷰가 발생한 가이드 | **15 / 38편** |
| 가이드 총 뷰 | 50 (사이트 전체 PV의 10%) |
| 가이드 랜딩 세션 | 34, 그중 direct(자기트래픽) 10 → **실제 외부 24세션 = 하루 0.9** |
| 28일 뷰 0인 가이드 | 19편 |
| 가이드 랜딩 세션당 PV | 1.44 (도구 랜딩 1.34) → **가이드→도구 이동이 사실상 없음** |
| 같은 기간 네이버 블로그 referral | **104세션** |

**글 한 편의 노력을 온사이트 가이드에 쓰면 24, 네이버 블로그에 쓰면 104다.** 4배 차이다.

원인도 명확하다. 가이드는 구글 색인을 전제로 설계됐는데 **구글이 두 달째 크롤을 안 온다.** 2026-09-07 카나리아 8개 재검에서 PASS는 홈 1개뿐이고, 나머지 7개의 마지막 크롤 날짜가 전부 2026-07-08~07-17에 멈춰 있다. GSC 90일 누적 노출 151·클릭 11(전부 7월 3주), 최근 28일은 노출 3·클릭 0이다.

**재개 조건 (둘 다 충족될 때만 이 게이트를 푼다)**
1. `node ~/.claude/analytics/check-indexing.mjs formda ~/formda/dist/sitemap.xml 8` 에서 **마지막 크롤 날짜가 7월에서 움직였을 것**. 기준선은 `~/.claude/analytics/reports/indexing-formda-2026-09-07.json`
2. 그 뒤 GSC 주간 노출이 회복될 것

1차 확인일은 **2026-10-05**다. 2026-09-07에 `SITE_MODIFIED`를 7/8→9/7로 올려 재빌드·배포하고([scripts/build.mjs](../../scripts/build.mjs) 137행), sitemap 재제출 + URL 검사 수동 색인 요청 10건(statement·receipt·estimate·invoice·expense-report·id-photo·loan·mandate·employment-contract·category/job)을 넣은 상태다. 크롤 날짜가 안 움직이면 lastmod 가설은 기각이고 구글 사이트단위 판단으로 확정한다.

**⚠️ 기존 38편은 삭제 금지.** 크롤이 돌아왔을 때 회수할 자산이다. 지금 지우면 그때 남는 게 없다.

### 왜 화·금 주 2편인가 (2026-08-21 설정)
가이드 34편이 2026-06-26~07-03에 몰려 나온 뒤(6/30 하루 24편) **7주간 0편**이었고, 구글 색인은 7/16부터 끊겼다. 인과를 단정할 수는 없으나 제이퍼가 대량생산으로 강등된 것과 모양이 같다([[project_jptcalc_google_collapse]]). 4편이라는 총량 자체는 작지만 **몰아 쓰지 않고 간격을 두는 것**이 이 큐의 전제다. 네이버는 발행 후 성숙에 1~2개월이 걸려 2주에 쓰든 4주에 쓰든 판정 시점이 같으므로, 간격을 좁혀 얻을 이득이 없다.
...
- [x] ~~유틸 도구 보류~~ → **2026-08-21 보류 해제(사용자 확정).** 아래 "유틸 짝 가이드 4편 큐" 참조. 해제 근거는 네이버 실측 수요(`blog/search-advisor.md`). **4편 한정이며 그 이상 확장 금지.**

### 유틸 짝 가이드 4편 큐 (2026-08-21 확정 · 화·금 주2편 · 4편에서 종료)

**배경**: 유틸 11종 중 짝 가이드가 없는 건 7종인데, 그중 네이버 실측 노출이 잡힌 4종만 쓴다. pdf-merge·pdf-watermark·text-align은 노출 기록이 없어 **대상 아님**(수요 없이 쓰면 대량생산).

**⚠️ 핵심 원칙 - 도구 페이지와 쿼리를 분리한다.** id-photo 도구 페이지는 "증명사진 규격 맞추기"(360노출)로 이미 나가 있고 **네이버 17위**다(2026-08-21 SERP 실측). 가이드가 같은 쿼리를 타면 자기잠식이고 같은 이유로 진다. 가이드는 **도구 페이지가 답하지 못하는 정보형 쿼리**를 탄다.

| 회차 | 날짜 | 도구 | 작성유형 | 도입부 | FAQ | 시각요소 | 글의 축 | 타깃 쿼리(실측) |
|---|---|---|---|---|---|---|---|---|
...
**회차별 진행 표기**: 완료 시 위 표 행 앞에 `[x] (발행일)`을 적고 실제 슬러그를 남긴다.

**종료 조건**: 4편을 마치면 **유틸 가이드는 추가하지 않는다.** 반응이 좋아도 이어붙이지 않는다(2026-06-30에 이틀간 33편을 몰아 쓴 뒤 7/16부터 구글 색인이 끊긴 전례). 효과 판정은 네이버 성숙 기간을 감안해 **2026-10월 초**, 9월 서치어드바이저 수확과 함께 한다.

## 주제 큐 — Phase 2: 런칭 후 (트래픽 쌓인 뒤 활성화)
네이버 서치어드바이저 검색어/웹문서 TOP30 + GA/`/report` 기반으로 큐를 월 1회 갱신, 먹힌 주제 더블다운(클러스터).

**실측 데이터 원본 = `blog/search-advisor.md`**(1차 수확 2026-08-12, 최근 30일 클릭 150·노출 5,900·CTR 2.5%). 아직 표본이 작아 큐를 전면 재편할 단계는 아니지만, 아래 두 가지는 이미 실측으로 잡힌다.

- **제목 최적화 우선 대상**(노출 큰데 CTR 1%대): ~~`id-photo`~~ **(2026-08-21 기각 - 네이버 SERP 17위라 CTR이 낮은 게 정상. 제목이 1위 검색어와 완전일치인데도 17위였고, 상위 경쟁사가 본문은 더 얇았다. 순위 문제라 제목으로 못 고친다)** → `how-to-write-e…`(315노출·1.0%) → `name-roman`(274노출·0.7%) → `mandate`(135노출·0.7%) → `how-to-write-s…`(133노출·0.8%). 슬러그가 잘린 항목은 `data/guides.js`에서 확정 후 손댈 것.
- **먹히는 클러스터**(CTR 상위): 금전·거래 증빙 축 - `invoice` 30.8% · `how-to-issue-payslip` 19% · `statement` 13.5% · `how-to-write-loan` 9.3% · `payment-pledge` 7.7%. 신규 가이드는 이 축에서 뽑는 것이 현재 근거상 유리.
- **미커버 실측 수요**: pdf-rotate(192노출)·image-to-pdf(224노출)·pdf-split(89노출) + id-photo(2,150노출) → **2026-08-21 보류 해제, 4편 큐로 확정**(위 "유틸 짝 가이드 4편 큐"). 4편 한정이며 [[feedback_formda_guide_new_tool_only]]에 따라 그 이상 연속 증산은 금지.

⚠️ 증가율(+22,611%)은 색인 램프 기저효과이지 성장이 아니다. 진짜 성장률 비교는 2026-09 수확부터.
